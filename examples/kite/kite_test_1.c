...
int main(int argc, char* argv[]){
	...
	tmax		= 2.0e5*2.*M_PI;	// in year/(2*pi)

	// INITIAL conditions
	// Parameters are those of Lee & Peale 2002, Figure 4. 
	struct reb_particle star = {0};
	star.m  = 1.00;			// This is a sub-solar mass star
	reb_add(r, star); 
	int j;
        int num_embryos = 70;
	struct reb_particle *p = calloc(sizeof(struct reb_particle),num_embryos);
        for(j=0;j<num_embryos;++j)
           {
	   double mass_mars = 3.213e-7;
	   double mu =  reb_random_uniform(0.5*mass_mars,2.0*mass_mars);
           double a = reb_random_powerlaw(1.0,10.0,0.5);
	   double e = reb_random_uniform(0.0,1e-4);
	   double i = reb_random_uniform(0.0,1e-4);
	   double w = reb_random_uniform(0.0,2.0*M_PI);
	   double O = reb_random_uniform(0.0,2.0*M_PI);
	   double f = reb_random_uniform(0.0,2.0*M_PI);
	   p[j] = reb_tools_orbit_to_particle(1.0, star, mu, a, e, i, O, w, f);
	   //p[j].r = 2.27e-5;
	   p[j].r = 2.27e-4;
	   reb_add(r, p[j]);
           }
	...
	tau_e = calloc(sizeof(double),r->N);
 	...
 	for(j=2;j<r->N;++j)  
	  {
	  tau_a[j] = 2.*M_PI*20000.0;	// Migration timescale of planet 2 is 20000 years.
//	  tau_e[j] = 2.*M_PI*0.0; 	// Eccentricity damping timescale is 200 years (K=0). 
	  tau_e[j] = 2.*M_PI*2000.0; 	// Eccentricity damping timescale is 200 years (K=10). 
//	  tau_e[j] = 2.*M_PI*200.0; 	// Eccentricity damping timescale is 200 years (K=100). 
	  }
	reb_move_to_com(r);  		
